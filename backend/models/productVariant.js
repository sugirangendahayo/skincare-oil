...
async function getVariantsByProductId(product_id) {
  const [rows] = await pool.query('SELECT * FROM product_variants WHERE product_id = ?', [product_id]);
  return rows;
}

async function createVariant(variantData) {
  const { product_id, variant_name, price_modifier, stock_quantity } = variantData;
